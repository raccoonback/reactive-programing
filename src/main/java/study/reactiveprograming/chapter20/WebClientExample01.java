...
import study.reactiveprograming.chapter20.BookDto.Response;
...
public class WebClientExample01 {
  ...
  private void exampleWebClient01() {
    ...
    response.subscribe(res -> {
      ...
      log.info("Header Location: {}", res.getHeaders().getLocation());
    });
  }
  ...
  private void exampleWebClient02() {
    ...
    Mono<BookDto.Response> response = webClient.patch()
    ...
    private void exampleWebClient03() {
      Mono<BookDto.Response> response = WebClient.create("http://localhost:8080")
          .get()
  ...
  private void exampleWebClient04() {
    Flux<Response> response = WebClient.create("http://localhost:8080")
        .get()
        .uri(uriBuilder -> uriBuilder.path("/v10/books").queryParam("page", "1").queryParam("size", "10").build())
        .retrieve()
